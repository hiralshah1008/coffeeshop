        ...
        <div class="container" id="page_start">
            ...
            <div class="row d-flex justify-content-center">
                <div class="menu-content pb-60 col-md-9">
                    <div class="title">
                        ...
                        <p><a style="color:#b68834!important;" target="_blank" href="/coffeeshop/public/addMenuItem/{{$shop->id}}"><h5 >Add Menu Item</h5></a></p>
                    </div>
                </div>
            ...
            <nav class="nav nav-pills flex-column flex-sm-row mb-50">
                @foreach($shop_array as $catname=>$v)
                    <span class="flex-sm-fill text-sm-center nav-link @if ($loop->first) navactive @endif" id="cat_{!! str_replace(' ', '', $catname) !!}"><strong>{{$catname}}</strong></span>
                @endforeach
            </nav>
            ...
            <div class="row">
                @if (!empty($shop_array)) 
                @foreach($shop_array as $catname=>$v)
                    <div class="row col-md-12" id="menu_{!! str_replace(' ', '', $catname) !!}">
...
@section('customjavascript')
    <script type="text/javascript">
        $(document).ready(function(){
            $('html,body').animate({
                 scrollTop: $("#page_start").offset().top - $('#header').outerHeight() - 30},
        'slow');
