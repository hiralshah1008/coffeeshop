...
class ShopController extends Controller
{
  ...
  /* Function to Add Shop */
    ...
    public function addMenu(Request $request, $shop_id){
        ...
        if($request->input('submit') != '' || $request->input('submit') == 1){
            ...
            if ($request->hasFile('item_image')) {
             ...
             return redirect('getShopDetail/'.$shop[0]->id);
    ...
    public function getAllShop($limit = null, $sortBy = null, $status = null){
        ...
        if(($limit != 'all' && !is_numeric($limit) && $limit != null) || ($sortBy != 'all' && $sortBy !='' && $sortBy != 'Asc'  && $sortBy != 'Desc') || ($status != 'all' && $status !='' && $status != 'ON'  && $status != 'CN')){
        ...
        $shops = DB::table('shop')->select('*')->whereIn('shop_status',['O','TC']);
